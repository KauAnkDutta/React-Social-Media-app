            ...
            if(req.body.userId !== req.params.id){
                try {
                    ...
                    if(!user.followers.includes(req.body.userId)){
                        ...
                        res.status(200).json({msg: `You area following the user ${req.body.userId}`})
                    }else{
                        return res.status(403).json({msg: "You are already following the user"})
                    }   
                } catch (error) {
            ...
            if(req.body.userId !== req.params.id){
                try {
                    ...
                    if(user.followers.includes(req.body.userId)){
                        ...
                        res.status(200).json({msg: `Unfollowing the user ${req.body.userId}`})
                    }else{
                        return res.status(403).json({msg: "You already unfollowed the user"})
                    }   
                } catch (error) {
